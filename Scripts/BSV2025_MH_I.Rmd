...
date: "Dezember 2018 / Januar 2019"
output:
...
```{r}
knitr::kable(arrange(tbl_schaden_zweckkat, desc(anteil_schadenanzahl)))%>%
...
```{r}
knitr::kable(arrange(tbl_schaden_zweckkat, desc(anteil_schadsum)))%>%
  kable_styling(bootstrap_options = "striped", full_width = F)
```
...
```{r}
knitr::kable(arrange(tbl_schaden_zweckkat, desc(mittlerer_schadensatz)))%>%
  kable_styling(bootstrap_options = "striped", full_width = F)
```
...
```{r}
knitr::kable(arrange(tbl_schaden_ursache, desc(anteil_schadenanzahl)))%>%
  kable_styling(bootstrap_options = "striped", full_width = F)
```
...
```{r}
knitr::kable(arrange(tbl_schaden_ursache, desc(anteil_schadsum)))%>%
  kable_styling(bootstrap_options = "striped", full_width = F)
```
...
16, 28, 32, 33, 79, 90 (dies sind auch die Kategorien welche die höchsten Schadengrade mit grossen Schadensamples aufweisen. Siehe Tabelle "Schadensatz nach Zweckcode"").
...
PC1 erklärt 59 % der Varianz, d.h. der Information aus den Daten mit vier Variablen lassen sich mit einer Hauptkomponente zusammenfassen.
...
tbl_zweck <- schad %>% filter(todesopfer > 0) %>% 
  group_by(zweckcode, zweck) %>%
  summarise(Brandfaelle = n()) %>%
  mutate(anteil = round(Brandfaelle / sum(schad$todesopfer > 0), 3) * 100)
...
tbl_zweck_ursache <- schad %>% filter(todesopfer > 0) %>% 
  ...
  summarise(Brandfaelle = n()) %>%
  mutate(anteil_am_total = round(Brandfaelle / sum(schad$todesopfer > 0), 3) * 100)
...
##### Brandfälle mit Todesopfern nach Zweckcode
```{r}
knitr::kable(arrange(tbl_zweck, desc(Brandfaelle)))%>%
  kable_styling(bootstrap_options = "striped", full_width = F)
```

##### Brandfälle mit Todesopfern nach Brandursache
```{r}
knitr::kable(arrange(tbl_ursache, desc(Brandfaelle)))%>%
  kable_styling(bootstrap_options = "striped", full_width = F)
```

##### Brandfälle mit Todesopfern nach Zweckcode und Brandursache
```{r}
knitr::kable(arrange(tbl_zweck_ursache, desc(Brandfaelle)))%>%
  kable_styling(bootstrap_options = "striped", full_width = F)
```

##### Brandfälle mit Todesopfern nach Baujahr
```{r}
knitr::kable(arrange(tbl_baujahr, desc(Brandfaelle)))%>%
  kable_styling(bootstrap_options = "striped", full_width = F)
```
